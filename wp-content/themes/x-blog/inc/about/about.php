			...
			'getting_started' => array(
                ...
                'seven' => array(
					...
					'recommended_actions'        => array(
						...
						'content'            => array(
							'click-to-top' => array(
								'title'       => __('Gallery Box', 'x-blog' ),
								'description' => __( 'These recommended plugin need to install and active for create awesome image/photo gallery, portfolio gallery, video gallery and more.', 'x-blog' ),
								'plugin_slug' => 'gallery-box',
								'id' => 'gallery-box'
							),
							'go-pro' => array(
								'title'       => '<a target="_blank" class="activate-now button button-danger" href="https://wpthemespace.com/product/'.$theme_slug.'">'.__('UPGRADE TO PRO','x-blog').'</a>',
...
//Admin notice 
function xblog_admin_notice__error() {
	if(!empty(get_option('xnotice6'))){
		delete_option( 'xnotice6' );
	}
    if(get_option('xnotice7')){
        return;
    }
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __( '<strong><span style="color:red"> UPGRADE FOR LIFETIME WITH NOMINAL PRICE.</span> <span style="color:green;">X BLOG PRO or X MAGAZINE PRO License Now Lifetime For You. Buy One Time And Get Lifetime Update, Advanced Features, SEO Advantage, High-Quality performance And support. So Why You late? Build Your Site With Pro Version 😃😄.</span> ', 'x-blog' );
    $url1 = esc_url('https://wpthemespace.com/product/x-blog/');
    $url2 =esc_url('https://wpthemespace.com/product/x-magazine/');

	printf( '<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-danger" href="%3$s" style="margin-right:10px">'.__('Upgrade X Blog pro','x-blog').'</a><a target="_blank" class="button button-danger" href="%4$s" style="margin-right:10px">'.__('Upgrade X Magazine pro','x-blog').'</a><button class="button button-info btnend" style="margin-left:10px">'.__('Dismiss the notice','x-blog').'</button></div>', esc_attr( $class ), wp_kses_post( $message ),$url1,$url2 ); 
}
add_action( 'admin_notices', 'xblog_admin_notice__error' );

function xblog_admin_notice_option(){
    if(isset($_GET['xnotice']) && $_GET['xnotice'] == 1 ){
        update_option( 'xnotice7', 1);
    }
}
add_action('init','xblog_admin_notice_option');